...
if __name__ == "__main__":
    ...
    with RPA.from_config(my_config) as rpa:
        ...
        if input("Do you want to prepare for match? y/n: ") == "y":
            presentation.prepare(90)

            if presentation.new_presentation is None:
                print("Matching did not start.")
            else:
                presentation = presentation.new_presentation
                ...
                while True:
                    ...
                    if ops is not None:
                        ...
                        if moqie:
                            time.sleep(0.1)
                            presentation.select_operation(op, randint(0, 13))  # noqa: S311
                        else:
                    ...
                    if presentation.new_presentation is not None:
                        ...
                        if isinstance(presentation, MatchPresentation):
                            continue
                        assert isinstance(presentation, TournamentPresentation)
                        break
